...
#define LOCTEXT_NAMESPACE "FFFengFFMPEGModule"

void FFFengFFMPEGModule::StartupModule()
{
	bInitialized = false;
...
void FFFengFFMPEGModule::FFmpegCallback(void*, int Level, const char* Format, va_list ArgList)
{
	char buffer[2048]{ 0 };
...
void FFFengFFMPEGModule::InitLibraryHandles()
{
...
void FFFengFFMPEGModule::UnloadHandledLibraries()
{
#if PLATFORM_WINDOWS
...
void* FFFengFFMPEGModule::LoadDependencyLibrary(const FString& DLLName)
{
	FString baseDir = IPluginManager::Get().FindPlugin("FFengFFMPEG")->GetBaseDir();
...
IMPLEMENT_MODULE(FFFengFFMPEGModule, FFengFFMPEG)